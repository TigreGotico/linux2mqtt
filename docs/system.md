...
telemetry from `psutil` (`USE_SYSTEM=true`, the default). Anything
unavailable on a host (no fan sensor, an unreadable mount, no load average)
...
| Disk *&lt;mount&gt;* Usage / Free | `disk_usage` | per `DISK_PATHS`, in % and GB. The SBC metric: a full SD card corrupts a Pi |
...
(`device_class: running`). The match is a substring of the process name and
the command line, so `homeassistant` matches a `python -m homeassistant`
process:
...
A container has its own PID namespace and filesystem, so by default the
process watch sees only the container's own processes, and disk usage
reflects the image's overlay. To watch the host, run with `--pid host` and
bind-mount the paths you want to report, for example `-v /:/host:ro` with
`DISK_PATHS=/host`.
...
---
[← Components](components.md) · [Home](index.md) · [Audio & media →](audio.md)
